...
void forwardBatch(NeuralNetwork* net, double** batch_images, int batchSize) {
    ...
    for (int i = 0; i < batchSize; i++) {
        for (int j = 0; j < INPUT_SIZE; j++) {
            flatBatchInput[j * batchSize + i] = batch_images[i][j];
        }
    }
...
void backwardBatch(NeuralNetwork* net, double** batch_images, double** batch_labels, int batchSize) {
    ...
    for (int i = 0; i < batchSize; i++) {
        for (int j = 0; j < OUTPUT_SIZE; j++) {
            flatBatchTarget[j * batchSize + i] = batch_labels[i][j];
        }
    }
...
int main() {
    printf("MNIST Neural Network (CUDA Implementation - Ultra Optimized)\n\n");
    int deviceCount;
